...
一个只面向手机的实时运动计数 Web App。用户选择动作、设置目标次数并允许相机后，应用会在设备本机运行 MediaPipe Pose Landmarker，把 33 个身体关键点绘制在镜像画面上，用共享动作状态机确认每一次完整动作，并通过声音和振动反馈计数。
...
要求 Node.js `^20.19.0` 或 `>=22.12.0`。从仓库根目录运行：
